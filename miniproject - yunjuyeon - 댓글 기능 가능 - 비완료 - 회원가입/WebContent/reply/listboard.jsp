...
if (key==null || keyword==null){
	strSQL = "SELECT count(*) FROM MIINIPROJECT2020";
}else{
	strSQL = "SELECT count(*) FROM MIINIPROJECT2020 WHERE " + key + " like '%" + keyword + "%'";
}
...
<center><font size='3'><b> 자유게시판 </b></font></TD>
...
if(lastRow > 0) {
	if(key==null || keyword==null){
		strSQL = "SELECT * FROM MIINIPROJECT2020 WHERE num BETWEEN " + startRow + " and " + endRow + "ORDER BY num ASC";
		rs = stmt.executeQuery(strSQL);
	} else {
		strSQL = "SELECT * FROM MIINIPROJECT2020 WHERE " + key + " like '%" + keyword + "%' ORDER BY num ASC";
		rs = stmt.executeQuery(strSQL);
	}
